    ...
    switch(info->event[i])
    {
      case EVENT_curtain:
        ...
        if(0x01 == currentDataPoint.valuecurtain)
        {
          //user handle
			LED1=0;
			OLED_ShowString(0,4,"CURT_ON ");
        }
        else
        {
          //user handle
			LED1=1;	
            OLED_ShowString(0,4,"CURT_OFF");	
        }
        break;
      case EVENT_lamplight:
        currentDataPoint.valuelamplight = dataPointPtr->valuelamplight;
        GIZWITS_LOG("Evt: EVENT_lamplight %d \n", currentDataPoint.valuelamplight);
        if(0x01 == currentDataPoint.valuelamplight)
        {
          //user handle
			LED2=0;
			OLED_ShowString(0,6,"LAMP_ON ");
        }
        else
        {
          //user handle 
			LED2=1;
            OLED_ShowString(0,6,"LAMP_OFF");	
        }
        break;
