...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flaglamplight)
    {
        info->event[info->num] = EVENT_lamplight;
        info->num++;
        dataPoints->valuelamplight = gizStandardDecompressionValue(lamplight_BYTEOFFSET,lamplight_BITOFFSET,lamplight_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuelamplight != cur->valuelamplight)
    {
        GIZWITS_LOG("valuelamplight Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(lamplight_BYTEOFFSET,lamplight_BITOFFSET,lamplight_LEN,(uint8_t *)devStatusPtr,dataPoints->valuelamplight);
